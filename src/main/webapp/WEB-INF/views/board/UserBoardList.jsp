			...
			$.ajax({
				...
				success : function(friendList){
					...
					for(var i in friendList){
						...
						//본인인지 확인
						if(friendList[i].mid == loginId){
							
						}else{
							output += "<button class='mofiBtn' style=' margin-top:auto; margin-bottom:auto; margin-left:50px; width:80px;' onclick='#'><i class='fas fa-comments'></i></i></button>";
							
							//친구인지 확인
							if(friendList[i].friendcheck == 0){
								output += "<button class='mofiBtn' style=' margin-top:auto; margin-bottom:auto; margin-left:8px; width:80px;' onclick='friendRequest(\""+friendList[i].mid+"\",\""+friendList[i].mnick+"\")'><i class='fas fa-user-plus'></i></button>";
							}else{
